...
class TestEventRegistrationViews(APITestCase):
    ...
    def test_event_registration_create(self):
        ...
        self.assertTrue(AdoptionEventRegistration.
                        objects.filter(event=self.event1,
                                       user=self.profile).exists())

    def test_event_registration_create_already_registered(self):
        """
        Test that user can only register once for an event.
        """
        AdoptionEventRegistration.objects.create(event=self.event1,
                                                 user=self.profile)
        url = reverse('registration-list', kwargs={'pk': self.event1.pk})
    ...
    def test_event_registration_delete(self):
        ...
        """
        registration = (AdoptionEventRegistration.
                        objects.create(event=self.event1, user=self.profile))
        url = reverse('registration-list', kwargs={'pk': registration.pk})
        response = self.client.delete(url)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(AdoptionEventRegistration.
                         objects.filter(pk=registration.pk).exists())
