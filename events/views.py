...
class UserEventRegistrations(APIView):
    ...
    def get(self, request):
        ...
        serializer = (AdoptionEventRegistrationSerializer(registrations,
                      many=True))
...
class EventRegistrationListCreateDelete(APIView):
    ...
    def get(self, request, pk):
        registrations = (AdoptionEventRegistration.
                         objects.
                         filter(user=request.user.profile, event_id=pk))
        serializer = (AdoptionEventRegistrationSerializer(registrations,
    ...
    def post(self, request, pk):
        ...
        if (AdoptionEventRegistration.
           objects.filter(event=event, user=request.user.profile).exists()):
        ...
        if serializer.is_valid():
            try:
                ...
                (logger.error(f"Error while saving registration: {e}",
                             exc_info=True))
