...
class Profile(models.Model):
  """
  Represents a user profile that extends the default Django User model by adding
  additional fields to it.

  Attributes:
    owner (User): A one-to-one relationship linking back to Django's User model.
    username (str): Username for the user, derived from the User model.
    ...
    zip_code (CharField): Postal code part of the user's address.
    has_other_pets (BooleanField): Indicates whether the user has other pets.
    has_children (BooleanField): Indicates whether the user has children.
  """
  ...
  class Meta:
    ...
    Attributes:
      ...
      verbose_name_plural (str): The human-readable name for multiple instances of the model.
    """
